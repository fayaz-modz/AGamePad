...
class _GamepadPageState extends State<GamepadPage> {
  ...
  Future<void> _saveLayout() async {
    if (_layout.id == 'xbox_default') {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Cannot modify default layout. Create a custom one.")));
      return;
    }
  ...
  Future<void> _editControl(GamepadControl control) async {
    ...
    Future<GamepadButton?> pickButton(GamepadButton? current, String label) async {
      return await showDialog<GamepadButton>(
        context: context,
        builder: (context) => SimpleDialog(
          title: Text('Select Mapping for $label'),
          children: GamepadButton.values.map((b) => SimpleDialogOption(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text(b.toString().split('.').last, style: const TextStyle(fontSize: 10, color: Colors.grey)),
              ],
            ),
          )).toList(),
        ),
    ...
    Future<Joystick?> pickJoystick(Joystick? current) async {
       return await showDialog<Joystick>(
        ...
        builder: (context) => SimpleDialog(
          ...
          children: Joystick.values.map((j) => SimpleDialogOption(
    ...
    if (control.type == ControlType.button || control.type == ControlType.shoulderButton) {
      ...
      if (selected != null) {
        setState(() {
           final index = _layout.controls.indexOf(control);
           if (index != -1) {
             _layout.controls[index] = control.copyWith(joystickMapping: selected);
           }
        });
      }
         ...
         builder: (ctx) => SimpleDialog(
           ...
           children: [
             ListTile(
               ...
               subtitle: Text(_descriptor.getButtonLabel(control.clusterBottom ?? GamepadButton.button1)),
               onTap: () async {
                 Navigator.pop(ctx);
                 final b = await pickButton(control.clusterBottom, 'Bottom Button');
                 if (b != null) setState(() {
                    final index = _layout.controls.indexOf(control);
                    if (index != -1) {
                      _layout.controls[index] = control.copyWith(clusterBottom: b);
                    }
                 });
               },
             ),
             ListTile(
               ...
               subtitle: Text(_descriptor.getButtonLabel(control.clusterRight ?? GamepadButton.button2)),
               onTap: () async {
                 Navigator.pop(ctx);
                 final b = await pickButton(control.clusterRight, 'Right Button');
                 if (b != null) setState(() {
                    final index = _layout.controls.indexOf(control);
                    if (index != -1) {
                      _layout.controls[index] = control.copyWith(clusterRight: b);
                    }
                 });
               },
             ),
             ListTile(
               ...
               subtitle: Text(_descriptor.getButtonLabel(control.clusterLeft ?? GamepadButton.button3)),
               onTap: () async {
                 Navigator.pop(ctx);
                 ...
                 if (b != null) setState(() {
                    final index = _layout.controls.indexOf(control);
                    if (index != -1) {
                      _layout.controls[index] = control.copyWith(clusterLeft: b);
                    }
                 });
               },
             ),
             ListTile(
               ...
               subtitle: Text(_descriptor.getButtonLabel(control.clusterTop ?? GamepadButton.button4)),
               onTap: () async {
                 Navigator.pop(ctx);
             ...
             ListTile(
               ...
               subtitle: Text(_descriptor.getButtonLabel(control.clusterC ?? GamepadButton.c)),
               onTap: () async {
                 Navigator.pop(ctx);
             ...
             ListTile(
               ...
               subtitle: Text(_descriptor.getButtonLabel(control.clusterZ ?? GamepadButton.z)),
               onTap: () async {
                 Navigator.pop(ctx);
    ...
    return Scaffold(
      ...
      body: Stack(
          children: [
            ...
            ..._layout.controls.map((control) => _buildControl(context, control, size)),
               ...
               Positioned(
                 ...
                 const Align(
                   ...
                   child: Padding(
                     ...
                     child: Text("EDIT MODE - Select a control to edit", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                   ),
                 ),
                           ...
                           child: Padding(
                             ...
                             child: Column(
                               ...
                               children: [
                                 ...
                                 Row(
                                   children: [
                                     const Text("Size", style: TextStyle(color: Colors.white70, fontSize: 10)),
                                     Expanded(
                                       child: SliderTheme(
                                         ...
                                         child: Slider(
                                           value: max(_selectedControl!.width, _selectedControl!.height),
                                           min: 0.05,
                                           max: 0.5,
                                 ...
                                 Row(
                                   ...
                                   children: [
                                     TextButton.icon(
                                       onPressed: () => _editControl(_selectedControl!),
                                     ...
                                     TextButton(
                                       ...
                                       child: const Text("Done", style: TextStyle(color: Colors.greenAccent, fontSize: 12)),
                                       style: TextButton.styleFrom(
                                         padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 0),
                                         minimumSize: Size.zero,
                                         tapTargetSize: MaterialTapTargetSize.shrinkWrap,
  ...
  Widget _buildControl(BuildContext context, GamepadControl control, Size screenSize) {
    ...
    final needsSquare = control.type == ControlType.dpad || 
    ...
    switch (control.type) {
      case ControlType.joystick:
         return VirtualJoystick(
           onChanged: (val) => _onJoystickChange(control.joystickMapping ?? Joystick.left, val),
         );
      ...
      case ControlType.button:
        return _OptionButton(
          ...
          onDown: () => _onButtonDown(control.buttonMapping ?? GamepadButton.select),
          onUp: () => _onButtonUp(control.buttonMapping ?? GamepadButton.select), 
        );
    }
  }
}
    ...
    return Listener(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          color: _isPressed ? Colors.blueAccent.withValues(alpha: 0.5) : Colors.grey[800],
          border: Border.all(
...
class _OptionButton extends StatefulWidget {
  ...
  const _OptionButton({required this.label, required this.onDown, required this.onUp});
